...
export class Landscape extends THREE.Mesh {
    ...
    constructor( o = {} ) {
        ...
        this.callback = typeof o.callback === "function" ? o.callback : noop;
        this.mapReady = false;
       ...
       // this.geometry.computeBoundingSphere();
        ...
        this.mapMax = maps.length * (isORM ? 3 : 2);

        for( var i in maps ){

            name = maps[i];
            txt[name] = this.loadTextures(assetUrl('./assets/textures/terrain/'+name+'.jpg'), { flip:false, repeat:this.uvx, encoding:o.encoding, callback: this.mapcallback.bind(this)  });
            txt[name+'_n'] = this.loadTextures(assetUrl('./assets/textures/terrain/'+name+'_n.jpg'), { flip:false, repeat:this.uvx, callback: this.mapcallback.bind(this) });
            if( isORM )txt[name+'_n'] = this.loadTextures(assetUrl('./assets/textures/terrain/'+name+'_n.jpg'), { flip:false, repeat:this.uvx, callback: this.mapcallback.bind(this) });
    ...
    mapcallback (){
        ...
        if( this.mapMax === 0 || this.mapN >= this.mapMax ){
            this.mapReady = true;
            this.callback();
        }
...
const TerrainShader = {
    ...
    rough : /* glsl */`
        ...
        float metalnessFactor = metalness;
            ...
            vec4 grassR = texture2D( roughnessMap1, vRoughnessMapUv );
            vec4 rockR = texture2D( roughnessMap2, vRoughnessMapUv );
    ...
    map : /* glsl */`
            ...
            vec4 sand = texture2D( map, vMapUv );
            vec4 grass = texture2D( map1, vMapUv );
            vec4 rock = texture2D( map2, vMapUv );
    ...
    normal : /* glsl */`
        #ifdef USE_NORMALMAP
            vec4 sandN =  texture2D( normalMap, vNormalMapUv );
            vec4 grassN = texture2D( normalMap1, vNormalMapUv );
            vec4 rockN = texture2D( normalMap2, vNormalMapUv );
