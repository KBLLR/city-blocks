...
const assetUrl = (path) => `/assets/${path.replace(/^(\.\/)?assets\//, "")}`;
...
export class SuperSky extends THREE.Mesh {
	...
	constructor( o = {} ) {
		...
		this.urls = [
			assetUrl('./assets/glsl/base_vs.glsl'),
			assetUrl('./assets/glsl/dome_fs.glsl'),
			assetUrl('./assets/glsl/sky_fs.glsl')
		];
		this.shaders = {};
		this.textureLoader = new THREE.TextureLoader();

		this.geometry = new THREE.SphereGeometry( this.size, 30, 15 );
    ...
    load() {
		if( this.urls.length === 0 ){ 
		    ...
		    // load noise map
			this.noiseMap = this.textureLoader.load( assetUrl("./assets/textures/sky/noise.png"), function ( texture ) { texture.wrapS = texture.wrapT = THREE.RepeatWrapping; texture.flipY = false; this.init(); }.bind(this) )
	...
	addLight() {
		...
		this.sunMaterial = new THREE.SpriteMaterial( { map: this.textureLoader.load(assetUrl("assets/textures/sky/lensflare1.png")), blending:THREE.AdditiveBlending, opacity:0.5 } );
        ...
        this.moonMaterial = new THREE.SpriteMaterial( { map: this.textureLoader.load(assetUrl("assets/textures/sky/lensflare2.png")), opacity:0.3 } );
		...
		var textureFlare3 = this.textureLoader.load(assetUrl("assets/textures/sky/lensflare3.png"))
		this.lensflare = new Lensflare();
		this.lensflare.addElement( new LensflareElement( this.textureLoader.load(assetUrl("assets/textures/sky/lensflare0.png")), this.size*0.1, 0, this.sun.color ) );
	...
	init(){
		...
		this.material = new THREE.ShaderMaterial( {
			...
			uniforms: {
                ...
                tDome: { value: this.textureLoader.load( assetUrl("./assets/textures/sky/milkyway.png"),  function ( t ) { t.encoding = THREE.sRGBEncoding; } ) },
			},
			vertexShader: this.shaders['base_vs'],
